...
def create_batch(h, T):
    ...
    for var in vars_list:
        batch_struct[var] = {
            ...
            't': np.zeros(num_steps) # On pré-remplit les temps aussi
        }
    return batch_struct

def raman_sim(k, X, h, T):
    """Stub pour Raman_Sim.m"""
    # Simulation factice : on ne fait rien ou on ajoute du bruit
    return X
...
def indpensim(f_input, Xd, x0, h, T, solv, p, Ctrl_flags):
    ...
    # Attention: en Python on modifie une copie locale de x0 ou l'objet lui-même
    current_pH_conc = 10**(-x0['pH']) 
    
    print(f"Démarrage simulation IndPenSim (N={N} pas)...")
    
    # Main loop
    # Python range va de 0 à N (inclus pour correspondre au MATLAB qui va de 1 à N+1 en taille)
    # Mais attention aux indices : k=0 correspond à t=0
    
    for k in range(len(t_vec)):
        # k correspond à l'index actuel.
        ...
        # On passe X, Xd, k (en index 1-based pour compatibilité logique si besoin), h, T
        # Note: f_input doit être converti pour accepter les dictionnaires Python
        # Le MATLAB fait: [u X] = f_input(...)
        u, X = f_input(X, Xd, k, h, T, Ctrl_flags)

        # Step 3: Build Initial Conditions for ODE (x00)
        # Indices MATLAB: 1-34.
        if k == 0:
            # Construction manuelle du vecteur d'état initial
            # [S, DO2, O2, P, V, Wt, pH, T, Q, Visc, Age, a0, a1, a3, a4, n0..n9, nm, phi0, CO2, CO2d, PAA, NH3, muP, muX]
            x00 = np.array([
            ...
            # Note: Le vecteur x00 doit faire la bonne taille (34 éléments dans le code MATLAB original)
            # Vérifions la taille des zeros : zeros(1,12) dans MATLAB -> 12 zéros.
            # Indices MATLAB : 
            # 1-15 (15 vars)
            # 16-27 (12 vars -> n0..n9, nm, phi0)
            # 28 (CO2outgas)
            # 29 (CO2d -> 0)
            # 30 (PAA)
            # 31 (NH3)
            # 32 (muP -> 0)
            # 33 (muX -> 0)
            # Total = 15 + 12 + 1 + 1 + 1 + 1 + 2 = 33 éléments ? 
            # MATLAB: [..., zeros(1,12), CO2outgas, 0, PAA, NH3, 0, 0]
            # Mon array numpy doit correspondre exactement.
            pass # La structure est créée implicitement par la concaténation ci-dessus
            
        else:
            # Récupération des valeurs précédentes (k-1)
            prev = k - 1
            x00 = np.array([
            ...
            # Note: MATLAB met 0, 0 à la fin pour mu_P et mu_X dans x00, car ils sont recalculés
        ...
        # Input Vector u00
        # Ordre MATLAB: [Inhib, Fs, Fg, RPM, Fc, Fh, Fb, Fa, h_ode, Fw, pressure, viscosity, Fremoved, Fpaa, Foil, NH3_shots, Dis, dist...]
        u00 = np.array([
            Ctrl_flags['Inhib'], u['Fs'], u['Fg'], u['RPM'], u['Fc'], u['Fh'], u['Fb'], u['Fa'],
            h_ode, u['Fw'], u['pressure'], u['viscosity'], u['Fremoved'], u['Fpaa'], u['Foil'],
            u.get('NH3_shots', 0), # Par sécurité
        ...
        # Modification des paramètres p 'in-place' si nécessaire
        if Ctrl_flags['Inhib'] in [1, 2] and k > 65:
            # MATLAB: diff((X.mu_X_calc.y((k-65:k-1),1)))
            # Slice numpy: [k-65 : k] (car k est exclus en fin de slice Python mais on veut jusqu'à k-1)
            mu_x_slice = X['mu_X_calc']['y'][k-65 : k]
            a1 = np.diff(mu_x_slice)
            a2 = a1 < 0
            if np.sum(a2) >= 63:
                # p(2) est mu_x max ? (Hypothèse selon le code MATLAB p(2) = ...)
                # Attention: p est un dictionnaire ou une liste ? Dans indpensim_run on a vu par=Parameter_list(...)
                # Supposons que p est un dictionnaire. Le code MATLAB utilise p(2), donc une liste/vecteur.
                # Faudra vérifier Parameter_list.m. Pour l'instant je suppose une liste.
                if isinstance(p, list) or isinstance(p, np.ndarray):
                    # Copie pour ne pas modifier la référence globale pour toujours si nécessaire
                    # Mais le code MATLAB modifie p pour l'appel ODE courant
                    p_current = p.copy() if isinstance(p, np.ndarray) else list(p)
                    p_current[1] = X['mu_X_calc']['y'][k-1] * 5 # Index 1 pour p(2)
                    p = p_current
                # Si p est un dict, il faudra adapter (ex: p['mux_max'])
        ...
        method = 'RK45'
        if solv == 2:
            method = 'BDF' # Equivalent ode15s
        elif solv == 3:
            method = 'Radau' # Equivalent ode23t (proche)
            
        # APPEL SOLVEUR
        sol = solve_ivp(indpensim_ode, t_span, x00, args=(u00, p), method=method)
        
        # Numerical stability (valeurs négatives -> 0.001)
        # sol.y est de forme (n_vars, n_steps). On prend la dernière colonne.
        y_final = sol.y[:, -1]
        ...
        # States (Mapping index 0-33 to Names)
        ...
        # Note: Gestion de division par zéro potentielle
        ...
        # Step 9: Advanced sensors (Raman / Pred)
        if k > 10:
        ...
        # MATLAB: rem(t, sample_rate) == 0. 
        # En flottant c'est dangereux. On utilise une tolérance.
        is_sampling_time = (np.isclose(t_final % Ctrl_flags['Off_line_m'], 0, atol=1e-3) or 
                            np.isclose(t_final, T, atol=1e-3) or 
                            np.isclose(t_final, 1, atol=1e-3)) # t=1 in matlab is weird but kept logic
        ...
        if is_sampling_time:
            ...
            if idx_delayed >= 0:
                ...
                X['X_offline']['t'][k] = X['X']['t'][idx_delayed]
                 ...
                 # Cas début de batch où le délai remonte avant 0
    ...
    # pH de concentration H+ vers log
    # Eviter log(0)
